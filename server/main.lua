...
if Config.UseRPName then
    ...
    elseif GetResourceState("qb-core"):find("start") then
        ...
        isPlayerAllowedToChangeName = function(source, notify)
...
local function getPlayerIdentifier(source)
    ...
    if not identifier then
        print(("[RadioList] Warning: No license identifier found for player %s"):format(source))
    end
    return identifier
end
...
local function setPlayerName(source, newName)
    ...
    if not playerIdentifier then
        print(("[RadioList] Error: Could not set name for player %s because identifier is nil"):format(source))
        return
    end
    ...
    customPlayerNames[playerIdentifier] = newName
